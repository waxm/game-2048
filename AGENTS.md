...
assets/app/game/game2048/domain/    # 纯规则、状态和数据模型
assets/app/game/game2048/config/    # 关卡与玩法配置
assets/app/game/game2048/controller/# 玩法流程编排
assets/app/game/game2048/view/      # 玩法表现组件
...
- 原通用约束：核心玩法场景默认命名为 `Game.scene`。
- 例外范围：`assets/scene/Game2048.scene`、`Game2048SceneController` 及对应场景清单。
- 原因：现有仓库使用带玩法语义的名称，能够避免多游戏资源迁移时产生歧义。
- 替代实现：仍保持 `Boot -> Lobby -> Game2048 -> Lobby`，全部切换通过 `SceneManager`。
- 验证方式：执行 `npm run verify:game`、`npm run validate:cocos` 并实际预览完整主流程。

### 现有业务目录

- 原通用约束：业务模块默认按 `domain/config/controller/view` 四层目录组织。
- 例外范围：`assets/app/game/game2048` 和现有 `assets/app/ui`。
...
- 每个游戏必须提交自己的 `GAME_UI_SPEC.json` 和 `tools/visual-review/cases.json`；框架同步只补齐缺失文件，不得覆盖游戏已经确认的视觉语言、布局令牌和页面状态。
- 视觉验收开始前必须刷新当前项目预览，截图只能来自浏览器 Canvas；必须同时记录当前项目标题、动态预览 URL、Canvas 尺寸、新鲜度线索和本次动作后的状态变化。
- 视觉证据只保存到已忽略的 `temp/cocos-workflow/visual-review`，不得提交截图、固定端口、标签 ID 或会话目录。
