...
- 设计尺寸：竖屏 `640 x 1136`。
- `master` 和 `dev` 只保存可复用框架，不保存任何具体小游戏业务。
- 本文件适用于框架分支以及从 `dev` 创建的所有游戏分支。

## 分支职责

- `master`：稳定框架版本，只接收已经验证完成的框架能力。
- `dev`：框架开发基线，新小游戏必须从这里创建独立分支。
- `codex/puzzle`：拼图游戏业务分支，包含完整拼图实现和资源。
- 新游戏分支不得把玩法代码、业务 UI、关卡配置或美术资源合回 `dev`、`master`。
- 游戏中产生的通用优化必须先从业务依赖中抽离，在 `dev` 单独实现和验证，再同步到 `master` 和需要它的游戏分支。
...
- 游戏分支中的业务逻辑放在 `assets/app/game`，UI、Prefab 和 Texture 按业务模块分类。
...
- 框架分支提交前执行 `npm run verify`。
- 游戏分支必须为玩法规则补充独立自动化测试，并把测试加入自己的 `verify` 命令。
